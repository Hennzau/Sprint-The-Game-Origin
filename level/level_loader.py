...
def build_level (level_json): #level_json format : 'data.json'
    ...
    level = Level(size,initial_positions_tuple , colors, final_positions_tuple)
    ...
    for obstacle in data["obstacles"]:
        ...
        if len(obstacle)==4 :
            Level.grid.obstacle[obstacle[0],obstacle[1]]=Obstacle(obstacle[2],obstacle[3])
        if len(obstacle)==5:
            Level.grid.obstacle[obstacle[0],obstacle[1]]=Obstacle(obstacle[2],obstacle[3],obstacle[4])
    return level
