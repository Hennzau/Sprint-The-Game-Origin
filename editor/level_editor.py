    ...
    def update(self, delta_time):
        if pygame.Rect(0, 0, self.size[0] * pixel_size, self.size[1] * pixel_size).collidepoint(pygame.mouse.get_pos()):
            if pygame.mouse.get_pressed()[0] and not self.select_add_player:
                ...
                if self.level.grid.obstacles[i, j] is None or (
                        self.level.grid.obstacles[i, j] is not None and not self.level.grid.obstacles[i, j].end and not
                self.level.grid.obstacles[i, j].start):
                    self.new_tile((i, j))
